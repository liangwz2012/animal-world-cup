...
async function main() {
  const required = [
    ...
    "runtime-assets/animal-cup/audio/ui_click.mp3",
  ...
  if (!generatedStandalone.includes("view:window.__animalCupScreenCanvas")) throw new Error("主 Canvas 显式注入未生效");
  ...
  if (!generatedStandalone.includes("physical device: skip dynamic fans atlas")) throw new Error("真机仍可能卡在动态观众图集生成");
  ...
  const mainFiles = allFiles.filter((file) => !file.startsWith(path.join(projectDir, "runtime-assets"))
    && !file.startsWith(path.join(projectDir, "tools"))
    && !file.startsWith(path.join(projectDir, "server"))
    && !file.endsWith("README.md")
    && !file.endsWith("package.json"));
  const subpackageFiles = allFiles.filter((file) => file.startsWith(path.join(projectDir, "runtime-assets")));
