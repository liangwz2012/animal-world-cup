...
function createLeaderboardClient(options = {}) {
  ...
  return {
    ...
    async submitResult(input) {
      return authed("POST", "/results", input);
    },
    async deleteAccount() {
    ...
    async fetchLeaderboard(metric) {
      if (!apiUrl) return { online: false, rows: [], self: null, metric: metric || "points" };
      // 已有会话时附带它以返回自己的名次；没有会话则保持公开查询，绝不为了看榜而额外弹登录。
      const result = await request(wxApi, {
        url: `${apiUrl}/leaderboards?metric=${encodeURIComponent(metric || "points")}`,
