...
export class FriendRoomServer {
  constructor(options = {}) {
    ...
    // 会话必须覆盖房间硬上限，否则长局断线后重连会被 SESSION_EXPIRED 拒绝，
    // 20 秒断线保留形同虚设。
    this.sessionTtlMs = Number(options.sessionTtlMs ?? this.hardTtlMs + 5 * 60 * 1000);
    ...
    this.checkCustomText = options.checkCustomText || (textSecurity.available ? textSecurity : null);
    this.rooms = new Map();
  ...
  async handleAuth(ws, message) {
    ...
    this.sessions.set(sessionHash, { userId: ws.friend.userId, expiresAt: this.now() + this.sessionTtlMs });
    ws.friend.sessionHash = sessionHash;
    this.send(ws, {
      type: "auth_ok",
  ...
  // 会话从登录起计时，房间从建房起计时。默认会话只比房间硬上限多 5 分钟，
  // 登录较久后才建房的玩家需要续期，才能覆盖房间全程和 20 秒断线恢复。
  renewSession(ws) {
    const hash = ws && ws.friend && ws.friend.sessionHash;
    if (!hash) return;
    const record = this.sessions.get(hash);
    if (record) record.expiresAt = this.now() + this.sessionTtlMs;
  }
  ...
  async handleCreateRoom(ws, message) {
    ...
    for (const [label, custom] of [[config.redLabel, config.redCustom], [config.blueLabel, config.blueCustom]]) {
      if (!custom) continue;
      if (!this.checkCustomText) throw new ProtocolError("CONTENT_CHECK_CONFIG_MISSING", "联网房间暂不接受未经安全检查的自定义队名");
      await this.checkCustomText({ content: label, openid: ws.friend.userId });
    }
    const now = this.now();
    ...
    // 会话从登录起计时、房间从建房起计时；建房时必须续期会话，
    // 否则登录较久的玩家在房间临到期前断线会被 SESSION_EXPIRED 挡掉。
    this.renewSession(ws);
