...
function makeGate(wx, extra = {}) {
  ...
  const gate = createPlayGate({
    ...
    enabled: extra.enabled != null ? extra.enabled : true,
    shareUnlockEnabled: extra.shareUnlockEnabled != null ? extra.shareUnlockEnabled : true,
    adUnlockEnabled: extra.adUnlockEnabled != null ? extra.adUnlockEnabled : true,
    ...extra.opts,
  });
  return { gate, setDay: (d) => { day = d; } };
}

// 0) 首发默认无限畅玩：没有云端广告开关时不得出现分享、广告或登录阻塞。
{
  const wx = makeWx();
  const gate = createPlayGate({ wxApi: wx });
  for (let i = 0; i < 12; i += 1) assert.equal(gate.tryConsume().ok, true);
  assert.equal(gate.enabled(), false);
}

// 1) 已开启广告模式时，每日免费 2 局，第 3 局被拦
{
  const wx = makeWx();
...
assert.equal(STORAGE_KEY, "rural-football:play-gate:v1");
