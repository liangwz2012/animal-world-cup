...
async function testRoomLifecycleAndRelay() {
  ...
  host.send("create_room", {
    requestId: "custom-without-checker",
    config: { redLabel: "青石村", redCustom: true },
  });
  assert.equal(
    (await host.next("error", (message) => message.requestId === "custom-without-checker")).code,
    "CONTENT_CHECK_CONFIG_MISSING",
    "未接入微信内容安全时，联网房间必须拒绝自定义队名",
  );
  ...
  assert.equal(created.room.config.redLabel, "镇隆");
  assert.equal(created.room.config.blueLabel, "水口");
...
async function testCustomLabelContentCheck() {
  const checked = [];
  const server = await createFriendRoomServer({
    port: 0,
    devAuth: true,
    checkCustomText: async (payload) => {
  ...
  const host = await TestClient.connect(server.url());
  await authDev(host, "custom-host");
  const created = await createRoom(host, {
    redLabel: "青石村",
    blueLabel: "水口",
    redCustom: true,
  ...
  assert.deepEqual(checked, [{ content: "青石村", openid: "dev:custom-host" }]);
  assert.equal(created.room.config.redLabel, "青石村");
  await host.close();
  await server.close();
}
...
async function testSessionRenewalCoversRoomLifetime() {
  let clock = 1_000_000;
  const server = await createFriendRoomServer({
    port: 0,
    devAuth: true,
  ...
  const defaults = new FriendRoomServer();
  assert.ok(
    defaults.sessionTtlMs >= defaults.hardTtlMs + 5 * 60 * 1000,
    "默认会话 TTL 必须至少覆盖房间硬上限加 5 分钟余量",
  );
  const host = await TestClient.connect(server.url());
  const auth = await authDev(host, "renew-host");
  // 登录 6 分钟后才建房：房间硬到期为登录后第 36 分钟，未续期的会话第 35 分钟就会失效。
  clock += 6 * 60 * 1000;
  const room = await createRoom(host);
  assert.ok(room.invite, "建房应成功");
  // 推进到房间硬到期前 20 秒（登录后第 35 分 40 秒），此时未续期的会话已经过期。
  clock += 30 * 60 * 1000 - 20_000;
  await host.close();
  const back = await TestClient.connect(server.url());
  back.send("auth", { requestId: "renew-reconnect", sessionToken: auth.sessionToken });
  const reauth = await back.next("auth_ok", (message) => message.requestId === "renew-reconnect");
  assert.ok(reauth.sessionToken, "房间临到期前的断线重连不得被 SESSION_EXPIRED 拒绝");
  await back.close();
  await server.close();
}

await testWxAuthBoundary();
await testCustomLabelContentCheck();
await testRoomLifecycleAndRelay();
await testLimitsAndExpiry();
await testHeartbeatTimeout();
await testSessionRenewalCoversRoomLifetime();
