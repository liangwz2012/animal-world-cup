...
await client.updateRegion({ code: "440100" });
...
assert.equal(calls.find((call) => call.url.endsWith("/region")).data.code, "440100");
...
const list = await client.fetchLeaderboard("points", "440000:city");
assert.equal(list.online, true);
assert.equal(list.rows[0].nickname, "广州");
assert.match(calls.find((call) => call.url.includes("/leaderboards")).url, /scope=440000%3Acity/);
