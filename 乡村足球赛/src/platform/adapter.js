...
function normalizeAssetPath(input) {
  ...
  if (path.startsWith("rural-football/")) return `${RUNTIME_ROOT}/${path}`;
  if (path.startsWith("app/match-runtime-min/")) return `${RUNTIME_ROOT}/${path.slice(4)}`;
  if (path.startsWith("app/rural-football/")) return `${RUNTIME_ROOT}/${path.slice(4)}`;
...
function publicUrlForLocalPath(localPath) {
  ...
  if (path.startsWith(`${RUNTIME_ROOT}/rural-football/`)) return "/rural-football/" + path.slice(`${RUNTIME_ROOT}/rural-football/`.length);
  return path;
}
...
function dynamicImageSource(localPath, global) {
  const registry = global && global.__RURAL_DYNAMIC_IMAGE_DATA_URIS__;
  if (!registry || typeof registry !== "object") return null;
  const original = String(localPath || "");
  const normalized = normalizeAssetPath(original);
  const candidates = [
    original,
    original.replace(/^\/+/, ""),
    normalized,
  ...
  for (const key of candidates) {
    if (Object.prototype.hasOwnProperty.call(registry, key) && registry[key]) return registry[key];
  }
  return null;
}

function localImageDataUri(localPath, wxApi, global) {
  const path = String(localPath || "");
  if (!path || /^data:/i.test(path) || /^https?:\/\//i.test(path) || path.indexOf("blob:") === 0) return null;
  // 队服文字等本地 Canvas 生成的图片只存在于内存注册表，先于文件系统查询。
  // 这样仍走既有 __rfSrc 路径，绝不触碰 wx 原生 image.src 属性描述符。
  const dynamicSource = dynamicImageSource(path, global);
  ...
  for (const key of candidates) {
    try {
      // 不要在这里调用 wx FileSystemManager.statSync：
      // DevTools 对分包资源偶发返回空统计对象，宿主的 statSync success callback
      // 会异步执行 Object.keys(null) 并抛错；该原生异步异常无法被这里的 try/catch
      // 捕获，最终会在 fans.load 阶段中断首帧。图片本来就必须读成 base64，
      // 因此直接用 base64 长度做同等的超大资源保护即可。
      const b64 = fs.readFileSync(key, "base64");
      if (b64 && b64.length > Math.ceil(IMAGE_DATA_URI_MAX_BYTES * 4 / 3) + 4) {
...
// __rfSrc 访问器（添加新属性是安全的），最终用普通赋值 img.src=最终值 交给原生。
// 构建期会把打包产物里的所有 `.src=` 赋值改写为 `.__rfSrc=`（见 tools/build.mjs），
// 非图片对象则由 Object.prototype 上的 __rfSrc 兜底访问器原样转发回 .src。
function installRfSrcFallback() {
  try {
    if (Object.getOwnPropertyDescriptor(Object.prototype, "__rfSrc")) return;
    Object.defineProperty(Object.prototype, "__rfSrc", {
      configurable: true,
      enumerable: false,
      get() { return this.src; },
...
function patchImage(img, global) {
  if (!img || img.__ruralFootballPatched) return img;
  safeSetGlobal(img, "__ruralFootballPatched", true);
  const listeners = {};
  ...
  const applySrc = (value) => {
    ...
    const dataUri = localImageDataUri(currentSrc, global.wx, global);
  ...
  try {
    Object.defineProperty(img, "__rfSrc", {
      configurable: true,
      enumerable: false,
...
function installBrowserApis(global, wxApi) {
  ...
  //   3) 静默全桩 —— 任何 create* 都有实现。
  {
    const NativeAudioContext = (typeof global.AudioContext === "function" && global.AudioContext)
      ...
      || null;
    if (NativeAudioContext && NativeAudioContext.__ruralFootballAudioShim) {
      // 热重载等场景下已装过本 shim，跳过
    } else {
    ...
    AudioContextShim.__ruralFootballAudioShim = true;
...
function installMiniWindow(options) {
  ...
  const ruralFootballEvents = new MiniEventTarget();
  safeSetGlobal(global, "__ruralFootballEvents", ruralFootballEvents);
  safeSetGlobal(global, "__ruralFootballCustomEvent", (type, options) => new MiniEvent(type, options));
  ...
  safeSetGlobal(global, "__ruralFootballLocation", miniLocation);
  ...
  const windowEvents = ruralFootballEvents;
  ...
  global.__ruralFootballResolvePath = normalizeAssetPath;
  global.__ruralFootballReadText = function readText(path) {
  ...
  global.__ruralFootballScreenCanvas = screenCanvas;
