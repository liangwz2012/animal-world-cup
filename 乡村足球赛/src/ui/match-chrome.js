...
function placardCharacters(value, maxLength = 6) {
  const cleaned = Array.from(String(value == null ? "" : value)
    .replace(/[\u0000-\u001f\u007f]/g, "")
    .replace(/\s+/g, "")
    .trim());
  return (cleaned.length ? cleaned : ["乡", "队"]).slice(0, Math.max(1, maxLength));
}

function sidelinePlacardLayout(value, height, scale) {
  const chars = placardCharacters(value);
...
function createMatchChrome(options) {
  ...
  root.name = "rural-football-match-chrome";
  const sidelineSignLayer = new PIXI.Container();
  ...
  function localityLabel(side) {
    const jersey = config && config[`${side}Jersey`] || {};
    const raw = jersey.customName || jersey.locationLabel || jersey.village || jersey.cityOrCounty || jersey.province || "乡队";
    const cleaned = Array.from(String(raw).replace(/[\u0000-\u001f\u007f]/g, "").replace(/\s+/g, "").trim());
    return (cleaned.length ? cleaned : ["乡", "队"]).slice(0, 6).join("");
  }

  function sideForTeam(teamId) {
    return teamId === config.blueTeam ? "blue" : "red";
  }

  function teamDisplayName(teamId) {
    return localityLabel(sideForTeam(teamId)) || teamName(teamId);
  }

  function teamDisplayRegion(teamId) {
    const side = sideForTeam(teamId);
    const region = config && config[`${side}Region`] || {};
    const path = Array.isArray(region.path) ? region.path : [];
    const label = path.slice(0, 2).map((item) => item && item.shortName).filter(Boolean).join(" · ");
    return label || teamCountry(teamId);
  }
  ...
  // 一字一牌：镇隆=两块，镇隆青年队=五块。每块牌尺寸保持一致，
  // 字数增加时沿边线自动扩展，不再在一块牌里压成难读的竖排小字。
  function drawSidelinePlacard(x, y, character, color, lightColor) {
    ...
    pole.endFill();
    pole.beginFill(0x406426, 0.9);
    if (typeof pole.drawEllipse === "function") pole.drawEllipse(x + signW / 2, y + signH + 8 * scale, 14 * scale, 3.5 * scale);
    else pole.drawRoundedRect(x + signW / 2 - 14 * scale, y + signH + 5 * scale, 28 * scale, 7 * scale, 4 * scale);
    pole.endFill();
  ...
  // 人物头像母版统一略朝左：左侧主队翻转为朝右，右侧客队保持朝左，形成面对面。
  redCaptainPortrait.scale.x = -Math.abs(redCaptainPortrait.scale.x || 1);
  blueCaptainPortrait.scale.x = Math.abs(blueCaptainPortrait.scale.x || 1);
  redCaptainPortrait.__ruralCaptainFacing = "right";
  blueCaptainPortrait.__ruralCaptainFacing = "left";
  const redName = center(text(teamDisplayName(config.redTeam), 15, 0xa44734, "900"), barX + 89 * scale, barY + 31 * scale);
  const blueName = center(text(teamDisplayName(config.blueTeam), 15, 0x315a9b, "900"), barX + barW - 89 * scale, barY + 31 * scale);
  ...
  rounded(scoreLayer, barX + barW / 2 - 29 * scale, barY + 14 * scale, 58 * scale, 34 * scale, 17 * scale, 0x5d9038, 0.95);
  ...
  function drawStats() {
    ...
    const rightTeam = center(text(teamDisplayName(config.blueTeam), 12, 0x315a9b, "900"), barX + barW - 72 * scale, barY + barH + 34 * scale);
  ...
  function captureScreenshot() {
    ...
    const done = (result) => {
      ...
      inputHost.__RURAL_FOOTBALL_LAST_SCREENSHOT__ = lastScreenshot;
  ...
  function showAbout() {
    ...
    const lines = [
      { t: compliance.gameName || "乡村足球赛", s: 22, c: 0x385823, w: "900" },
      { t: "底层足球引擎基于 NeoXu954 开源项目改造", s: 15, c: 0x5a6a4c, w: "700" },
      { t: "源码与许可  github.com/NeoXu954/animal-world-cup", s: 14, c: 0x5a6a4c, w: "700" },
  ...
  function showResult(detail) {
    ...
    generateMatchShareCard(wxApi, {
      ...
      blueName: teamDisplayName(config.blueTeam),
      redCountry: teamDisplayRegion(config.redTeam),
      blueCountry: teamDisplayRegion(config.blueTeam),
  ...
  function storePointerDiagnostic(value) {
    inputHost.__RURAL_FOOTBALL_MATCH_LAST_TOUCH__ = value;
    if (inputHost.window) inputHost.window.__RURAL_FOOTBALL_MATCH_LAST_TOUCH__ = value;
    if (typeof globalThis !== "undefined") globalThis.__RURAL_FOOTBALL_MATCH_LAST_TOUCH__ = value;
  }
  ...
  function handlePointer(event) {
    ...
    for (const candidate of candidates) {
      ...
      for (let index = hitAreas.length - 1; index >= 0; index -= 1) {
        ...
        if (x >= hit.x && x <= hit.x + hit.w && y >= hit.y && y <= hit.y + hit.h) {
          ...
          console.info("[rural-football-match] TOUCH_HIT", hit.kind, candidate.ratio.toFixed(2), Math.round(x), Math.round(y));
          hit.action();
          return;
        }
      }
    ...
    console.warn("[rural-football-match] TOUCH_MISS", JSON.stringify({ raw, candidates, resultVisible }));
  }
