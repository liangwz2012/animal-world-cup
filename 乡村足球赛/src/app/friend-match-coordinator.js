...
function roomWireConfig(input) {
  ...
  return {
    ...
    redCustom: !!(config.redRegion && config.redRegion.customName),
    blueCustom: !!(config.blueRegion && config.blueRegion.customName),
  };
}

function mergeRoomConfig(base, wire, roomId) {
...
function createFriendMatchCoordinator(options) {
  ...
  function shareInvite() {
    ...
    const payload = sharePayload({
      imageUrl: globalObject.__RURAL_FOOTBALL_LAST_SCREENSHOT__ || undefined,
    });
  ...
  function executeIntent(intent) {
    ...
    pendingIntent = null;
    if (intent.type === "create") return client.createRoom(roomWireConfig(intent.config));
  ...
  function bindClient(nextClient) {
    ...
    nextClient.on("room_created", (message) => {
      ...
      if (room && room.config) frozenConfig = mergeRoomConfig(frozenConfig, room.config, room.roomId);
  ...
  function beginSynchronizedMatch(message, nextRole, sessionKind, startPaused) {
    const config = mergeRoomConfig(frozenConfig, message.config || frozenConfig, message.roomId || room && room.roomId || "");
  ...
  function handleRoomState(message) {
    ...
    if (room.config) frozenConfig = mergeRoomConfig(frozenConfig, room.config, room.roomId);
